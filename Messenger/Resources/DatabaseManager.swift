...
/// Manager object to reald and write data to real time firebase database
...
// MARK: - Sending messsages / conversation
...
extension DatabaseManager {
        ...
        ref.observeSingleEvent(of: .value) { [weak self] snapshot in
            ...
            if var conversations = userNode["conversations"] as? [[String: Any]] {
                ...
                ref.setValue(userNode) { [weak self] error, _ in
                    ...
                    self?.finishCreatingConversation(convrsationId: conversationID, name: name, firstMessage: firstMessage, completion: completion)
                }
            } else {
    ...
    private func finishCreatingConversation(convrsationId: String, name: String, firstMessage: Message, completion: @escaping (Bool) -> Void) {
        //    {
        //      "id": String,
        //      "type": text, photo, video,
        //      "content": String.
        ...
        print("adding convo: \(convrsationId)")
        
        database.child("\(convrsationId)").setValue(value) { error, _ in
            guard error == nil else {
                completion(false)
                return
            }
    ...
    public func getAllConversations(for email: String, completion: @escaping (Result<[Conversation], Error>) -> Void) {
        database.child("\(email)/conversations").observe(.value, with: { snapshot in
            ...
            let conversations: [Conversation] = value.compactMap({ dictionary in
                ...
                let latestMmessageObject = LatestMessage(date: date, text: message, isRead: isRead)
                return Conversation(id: conversationId, name: name, otherUserEmail: otherUserEmail, latestMessage: latestMmessageObject)
            })
    ...
    public func getAllMessagesForConversation(with id: String, completion: @escaping (Result<[Message], Error>) -> Void) {
        database.child("\(id)/messages").observe(.value) { snapshot in
            ...
            let messages: [Message] = value.compactMap { dictionary in
                guard let content = dictionary["contact"] as? String,
                ...
                if type == "photo" {
                    ...
                    guard let videoUrl = URL(string: content),
                          let placeholder = UIImage(named: "plaseholder") else { return nil}
                    let media = Media(url: videoUrl, image: nil, placeholderImage: placeholder, size: CGSize(width: 300, height: 300))
                    kind = .video(media)
                } else if type == "location" {
                    let locationComponents = content.components(separatedBy: ",")
                    guard let latitude = Double(locationComponents[0]), let longitude = Double(locationComponents[1]) else { return nil }
                        ...
                        strongSelf.database.child("\(otherUserEmail)/conversations").observeSingleEvent(of: .value) { snapshot in
                            ...
                            if var otherUserConversations = snapshot.value as? [[String: Any]]  {
                                ...
                                if var targetConversation = targetConversation {
                                    ...
                                    // faild to find in current collection
                                    let newConversationData: [String: Any] = [
                                        "id": conversation,
    ...
    public func conversationExsits(with targetRecipientEmail: String, completion: @escaping (Result<String, Error>) -> Void) {
